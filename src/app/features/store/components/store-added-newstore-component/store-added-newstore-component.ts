...
export class StoreAddedNewstoreComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder , private storeService: StoreServices , private router: Router) {
  ...
  onSubmit() {
    if (this.storeDataForm.valid) {
      ...
      this.storeService.addStore(this.storeDataForm.value).subscribe({
        next: (response) => {
